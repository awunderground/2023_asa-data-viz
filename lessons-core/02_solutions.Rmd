...
# step 4
ggplot(data = storms) + 
  geom_point(mapping = aes(x = pressure, y = wind), alpha = 0.1)
...
ggplot(data = msleep) +
  ...
  scale_y_continuous(limits = c(0, NA), breaks = seq(from = 0, to = 20, by = 10))
...
```{r}
my_plot <- ggplot(data = storms, mapping = aes(x = pressure, y = wind)) + 
  geom_point() +
  geom_smooth()
...
ggsave(filename = "day02/favorite-plot.png", width = 6.5, height = 4)
